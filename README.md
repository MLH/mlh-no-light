...
[![Circle CI](https://circleci.com/gh/bih/no-light/tree/master.svg?style=svg)](https://circleci.com/gh/MLH/no-light/tree/master) [![Coverage Status](https://coveralls.io/repos/bih/no-light/badge.svg?branch=master&service=github)](https://coveralls.io/github/bih/no-light?branch=master)
...
Jon's excited to show the submissions. He visits the link that was given to Jane and Sam and he added `.zip` to the end (so the link was `http://no-light.mlh.io/whatever-this-event-is-called`). He is now given a ZIP file of all the submissions. Amazing.

Jon opens each file and picks one winner through an audience vote. We're not sure it works if there is two participants, but a story with too many characters doesn't sound too appealing either.
